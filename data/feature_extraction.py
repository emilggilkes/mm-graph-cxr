...
class FeatureExtraction():
    def __init__(self) -> None:
        ...
        self.imageroot = '/n/holyscratch01/protopapas_lab/Everyone/eghitmangilkes/data/mimic/files/' # '/home/agun/mimic/dataset/VG/data/'
        # self.reportroot = '/home/agun/mimic/dataset/VG/MIMIC-CXR-Reports/'
        self.path = '/n/holyscratch01/protopapas_lab/Everyone/eghitmangilkes/data/imagenome/chest-imagenome/silver_dataset/feature_extraction_final/' # '/home/agun/mimic/dataset/VG/FeatureData/'
        self.outputdir = '/n/holyscratch01/protopapas_lab/Everyone/eghitmangilkes/data/imagenome/chest-imagenome/silver_dataset/coco/' # "/home/agun/mimic/dataset/VG/"
        self.cocodir = '/n/holyscratch01/protopapas_lab/Everyone/eghitmangilkes/data/imagenome/chest-imagenome/silver_dataset/coco/updated_final/' 
    ...
    def setup(self):
        ...
        path = '/n/holyscratch01/protopapas_lab/Everyone/eghitmangilkes/obj-detect/output/'
        cfg.MODEL.WEIGHTS = os.path.join(path, 'model_0119999.pth')
    ...
    def get_board_dicts(self,imgdir):
        ...
        for i in dataset_dicts: # tqdm(dataset_dicts, desc="get_board_dict"):
    ...
    def load_dict(self,imgdir):
        ...
        with open(json_file) as f:
            ...
            # print(len(dataset_dicts))
        data_dict = {}
        for images in dataset_dicts: # tqdm(dataset_dicts, desc="load_dict"):
    ...
    def main(self,predictor):
        for batch in ["train"]:
            save_file_name = os.path.join(self.cocodir, "xray_graph_{}.json".format(batch)) # "/home/agun/mimic/dataset/VG/xray_graph_{}.json".format(batch)
            save_file_name2 = os.path.join(self.cocodir, "xray_image_{}.json".format(batch))  # "/home/agun/mimic/dataset/VG/xray_image_{}.json".format(batch)
            print('Loading training dictionaries')
            train_dict = FeatureExtraction().load_dict(os.path.join(self.outputdir, "xray_coco_{}.json".format(batch))) # load_dict("/home/agun/mimic/dataset/VG/xray_coco_{}.json".format(batch)))
            print('Get board dictionaries')
            dataset_dicts = FeatureExtraction().get_board_dicts(os.path.join(self.outputdir, "xray_coco_{}.json".format(batch))) # get_board_dicts("/home/agun/mimic/dataset/VG/xray_coco_{}.json".format(batch))
            ...
            chunk_size = (len(dataset_dicts) // 4)
            for d in tqdm(dataset_dicts[:chunk_size]): # tqdm(dataset_dicts, desc="file processing"):
                count += 1
                ...
                if len(annotations) < 18:
                    target = {i: list(np.zeros(len(self.diseaselist), dtype=int)) if i not in annotations.keys() else annotations[i] for i in range(len(self.organs))}
                else:
                    target = annotations
                target = dict(collections.OrderedDict(sorted(target.items())))   
                target = [list(map(int, target[i])) for i in target.keys()]
                for bbox, obj, probs, feature in zip(bboxes, objects, cls_prob, features):
                    
                    # try:
                    #     things = annotations[obj]
                    # except:
                    #     print("Target for {} not in File {}".format(str(organs[obj]), image_id))
                    #     # annotation = np.ones([len(diseaselist)]) * -1 # annotation = np.zeros([len(diseaselist)])
            ...
            coco_data.to_csv(os.path.join(self.cocodir, "new_{}_1.csv".format(str(batch))), sep='\t', index=False)
