...
if __name__=="__main__":
    ...
    zero_feature_dir = '/n/holyscratch01/protopapas_lab/Everyone/eghitmangilkes/data/imagenome/chest-imagenome/silver_dataset/feature_extraction_zero/'
    ...
    for filename in feature_files:
        ...
        if np.array(img_feats['target']).shape[0] < 18:
            ...
            for i in missing_objs_idx:
                if t.shape[0] == 0:
                    t = (np.zeros(len(diseaselist))).reshape(1,-1)
                elif i == 0:
                    t = np.vstack([np.zeros(len(diseaselist)), t])
                elif i == 17:
                    t = np.vstack([t, np.zeros(len(diseaselist))])
                else:
                    t = np.vstack([t[:i], np.zeros(len(diseaselist)), t[i:]])
        #     print(i)
            img_feats['target'] = t.tolist()
        with open(os.path.join(zero_feature_dir, filename), 'w', encoding='utf-8') as f:
