...
if __name__=="__main__":
    ...
    mm_feature_dir = '/n/holyscratch01/protopapas_lab/Everyone/eghitmangilkes/data/imagenome/chest-imagenome/silver_dataset/mm_feature_extraction_final_fulltext/'
    ...
    for filename in tqdm(feature_files[:118922]):
        ...
        report_phrases = {}
        for attribute in img_scene['attributes']:
            if len(attribute['phrases']) > 0:
                for i, id in enumerate(attribute['phrase_IDs']):
                    # print(id)
                    report_phrases[id.split('|')[1]] = attribute['phrases'][i]
        report_phrases_sorted = {i[0]: i[1] for i in sorted(report_phrases.items())}

        img_feats['report_text'] = report_phrases_sorted
