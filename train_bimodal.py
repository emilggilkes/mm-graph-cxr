...
from model.loss import ranking_loss, sigmoid_focal_loss
...
parser.add_argument('--max_grad_norm', default=2.0, type=float, help='for gradient clipping')
...
parser.add_argument('--focal', action='store_true', help='Whether to use focal loss function')
...
class_weights = torch.Tensor(pd.read_csv('/n/holyscratch01/protopapas_lab/Everyone/eghitmangilkes/data/imagenome/chest-imagenome/silver_dataset/class_counts/class_weights.csv').values)
...
def train_epoch(rank, model, train_dataloader, valid_dataloader, img_loss_fn, text_loss_fn, optimizer, scheduler, writer, epoch, args):
    model.train()
    with tqdm(total=len(train_dataloader), desc='Step at start {}; Training epoch {}/{}'.format(args.step, epoch+1, args.n_epochs)) as pbar:
        for image_features, text_features, target, image_id in train_dataloader:
            # text_mask = report_tokens['attention_mask'].to(rank)
            # text_input_id = report_tokens['input_ids'].squeeze(1).to(rank)
            text_inputs = text_features.to(rank)
            # print("image_features shape:", image_features.size())
            img_inputs = image_features.to(rank)
            #print("text_input_id", text_input_id.size())
            target_data = target.to(rank)
            args.step += 1
            embedding_img, logits_img, embedding_text, logits_text = model(img_inputs, text_inputs)
            # pooled_output, logits = model(text_input_id, text_mask)
            if args.focal:
                text_loss = text_loss_fn(logits_text, target_data, reduction='none', alpha=class_weights.to(rank), gamma=args.focal_gamma).mean()
                img_loss = img_loss_fn(logits_img, target_data, reduction='none', alpha=class_weights.to(rank), gamma=args.focal_gamma).mean()
            else:
                text_loss = text_loss_fn(logits_text, target_data).mean() #.sum(1).sum(1).mean(0)
                img_loss = img_loss_fn(logits_img, target_data).mean()
                
            # print("mean_text_loss", text_loss.size())
            # print("mean_img_loss", img_loss.size())
            # print("loss1", torch.isnan(loss).any() )
            # loss = torch.mean(loss)
            joint_loss = ranking_loss(embedding_img, embedding_text, target_data, image_id,
                    similarity_function='dot').mean()
            # print("joint_loss", joint_loss.size())
            # model.parameters.sum()
            optimizer.zero_grad()
            loss = img_loss + text_loss + joint_loss
...
def evaluate(rank, model, dataloader, loss_fn, args):
    ...
    with tqdm(desc='Evaluation', unit='it', total=len(dataloader)) as pbar:
        for image_features, text_features, target, image_id in dataloader:
            img_inputs = image_features.to(rank)
            # text_mask = report_tokens['attention_mask'].to(rank)
            # text_input_id = report_tokens['input_ids'].squeeze(1).to(rank)
            target_data = target.to(rank)
            ...
            with torch.no_grad():
                # embedding_img, logits_img, embedding_text, logits_text = model(img_inputs, text_input_id, text_mask)
                embedding_img, logits_img = model.module.img_model(img_inputs)
            
            if args.focal:  
                loss = loss_fn(logits_img, target_data, reduction='none', alpha=args.focal_alpha, gamma=args.focal_gamma)
            else:
                loss = loss_fn(logits_img, target_data)  
            outputs += [logits_img.cpu()]
            targets += [target]
            losses  += [loss.cpu()]
...
def train_and_evaluate(rank, model, train_dataloader, valid_dataloader, test_dataloader,  img_loss_fn, text_loss_fn, optimizer, scheduler, writer, args):
    for epoch in range(args.n_epochs):
        # train
        train_epoch(rank, model, train_dataloader, valid_dataloader,  img_loss_fn, text_loss_fn, optimizer, scheduler, writer, epoch, args)

        # evaluate
        print('Evaluating...', end='\r')
        eval_metrics = evaluate_single_model(rank, model, test_dataloader, img_loss_fn, args)
...
def main(rank, args, world_size):
    ...
    args.output_dir = '/n/holyscratch01/protopapas_lab/Everyone/eghitmangilkes/bimodal-fulltext-dualanax-output-focal-weights-1/'
    ...
    train_dataloader, train_dataset = get_loader(rank, root_folder=path,mode='train', tokenizer=tokenizer, num_workers=world_size,batch_size=args.batch_size, shuffle=True, world_size=world_size)
    ...
    test_dataloader, test_dataset = get_loader(rank, root_folder=path,mode='test', tokenizer=tokenizer, num_workers=world_size,batch_size=args.batch_size, shuffle=False, world_size=world_size)
    ...
    #             in_channel1=300, 
    #             in_channel2=1024)
    # text_model = RadBERT("StanfordAIMI/RadBERT", n_classes, dropout=0.1)
    ...
    #     param.requires_grad = False
    model = BimodalModel(n_classes=n_classes, embedding_size=1024)
    # for param in model.text_model.base_model.parameters():
    #     param.requires_grad = False
    ...
    if args.focal:
        print('Using focal loss function.')
        img_loss_fn = sigmoid_focal_loss
        text_loss_fn = sigmoid_focal_loss
    else:
        print('Using BCEWithLogitsLoss.')
        img_loss_fn = nn.BCEWithLogitsLoss(reduction='none').to(rank)
        text_loss_fn = nn.BCEWithLogitsLoss(reduction='none').to(rank)
    
    
    if args.train:
        train_and_evaluate(rank, model, train_dataloader, valid_dataloader, test_dataloader, img_loss_fn, text_loss_fn, optimizer, scheduler, writer, args)
    
    if args.evaluate_single_model:
        eval_metrics = evaluate_single_model(model, valid_dataloader, img_loss_fn, args)
